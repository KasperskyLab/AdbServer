...
internal class DesktopDeviceSocketConnectionForwardImpl : DesktopDeviceSocketConnection {
    ...
    override fun getDesktopSocketLoad(executor: CommandExecutor): () -> Socket {
        ...
        logger.i("getDesktopSocketLoad", "calculated desktop client port=$clientPort")
        forwardPorts(executor, clientPort, DEVICE_PORT)
        logger.i("getDesktopSocketLoad", "desktop client port=$clientPort is forwarding with device server port=$DEVICE_PORT")
        return {
            logger.i("getDesktopSocketLoad", "started with ip=$LOCAL_HOST, port=$clientPort")
            val readyClientSocket = Socket(LOCAL_HOST, clientPort)
            logger.i("getDesktopSocketLoad", "completed with ip=$LOCAL_HOST, port=$clientPort")
            readyClientSocket
        }
    }
    ...
    private fun forwardPorts(executor: CommandExecutor, fromPort: Int, toPort: Int) {
        logger.i("forwardPorts(fromPort=$fromPort, toPort=$toPort)", "started")
        val result = executor.execute(AdbCommand("forward tcp:$fromPort tcp:$toPort"))
        logger.i("forwardPorts(fromPort=$fromPort, toPort=$toPort)", "result=$result")
    }

    override fun getDeviceSocketLoad(): () -> Socket = {
        logger.i("getDeviceSocketLoad", "started")
        val serverSocket = ServerSocket(DEVICE_PORT)
        val readyServerSocket = serverSocket.accept()
        logger.i("getDeviceSocketLoad", "completed")
        readyServerSocket
    }
}